...
class PlayerDemo {
    ...
    private videoElement: HTMLVideoElement = <HTMLVideoElement>document.querySelector('#video-player');
    ...
    constructor(player: MediaPlayer) {
        ...
        const versionString: HTMLElement = <HTMLElement>document.querySelector('.version');
...
const inputEl: HTMLInputElement = <HTMLInputElement>document.querySelector('.src-input');
let demo: PlayerDemo;
...
function setupForm() {
    ...
    if (formEl) {
        formEl.addEventListener('submit', (event: any) => {
